    ...
    override fun apply(target: Project) {
        with(target) {
            ...
            val androidExtension = when {
                isAndroidLib() -> extensions.getByType<LibraryExtension>()
                isAndroidApp() -> extensions.getByType<BaseAppModuleExtension>()
                else -> throw IllegalStateException("applicable only to AndroidModule")
            }
            ...
            // ToDo PS-104
            when {
                isAndroidLib() -> configureJacoco(extensions.getByType<LibraryAndroidComponentsExtension>())
                isAndroidApp() -> configureJacoco(extensions.getByType<ApplicationAndroidComponentsExtension>())
            }
        }
    }
}
